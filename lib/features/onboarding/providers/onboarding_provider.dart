...
class OnboardingState {
  const OnboardingState({required this.hasCompleted, this.selectedUseCase});
...
class OnboardingNotifier extends _$OnboardingNotifier {
  ...
  OnboardingState _loadFromPrefs(SharedPreferences prefs) {
    ...
    final useCase = useCaseRaw != null
        ? UseCaseType.values.firstWhereOrNull((e) => e.name == useCaseRaw)
        : null;
  ...
  Future<void> completeOnboarding() async {
    ...
    AppLogger.i(
      'OnboardingProvider',
      'Onboarding complete. Default model: $defaultModel',
    );

    // Update state LAST so the app only moves on once saving is completely done
